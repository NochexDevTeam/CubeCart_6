...
<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
	<div id="Nochex_APC" class="tab_content">
  		<!--h3>{$TITLE}</h3-->
		<img src="https://www.nochex.com/logobase-secure-images/logobase-banners/clear-amex-mp.png" alt="Nochex" style="height:100px;" />
  		<fieldset><legend>{$LANG.module.cubecart_settings}</legend>
			
			<div><label for="product_id">{$LANG.nochex_apc.mode_merchantid}</label><span><input name="module[email]" id="email" class="textbox" type="text" value="{$MODULE.email}" /></span></div>
  			<div><label for="email">{$LANG.nochex_apc.mode_test}</label><span><input type="hidden" name="module[testMode]" id="testMode" class="toggle" value="{$MODULE.testMode}" /></span>
    		</div>
